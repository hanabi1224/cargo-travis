...
fn execute(options: Options, config: &mut Config) -> CliResult {
    debug!(
        "executing; cmd=cargo-coverage; args={:?}",
    ...
    let ws = if let Some(path) = options.flag_manifest_path {
        ...
        let root = try!(cargo::util::important_paths::find_root_manifest_for_wd(
            config.cwd()
        ));
    ...
    let spec = try!(Packages::from_flags(
        options.flag_all,
    ...
    let mut build_config = try!(BuildConfig::new(
        config,
        options.flag_jobs,
        &options.flag_target,
        mode
